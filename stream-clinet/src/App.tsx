...
import { streamFiles, downloadFiles } from "./services/fileStreamService";
...
export default function App() {
  ...
  async function handleDownload() {
    ...
    await downloadFiles(secret, (done, total, current) => {
      setProgress({ done, total, current });
    });
  }
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center font-sans">
      <div className="w-105 rounded-2xl border border-[#2a2d3e] bg-[#1a1d27] p-10 shadow-[0_8px_40px_rgba(0,0,0,0.5)]">
        <h1 className="m-0 text-2xl font-bold text-[#e8eaf6]">File Streamer</h1>
        ...
        <div className="w-full">
          <button className={primaryBtn+" w-full"} onClick={handleDownload}>
            Download
          </button>
        </div>
